...
## Acceptance Criteria
- [ ] `variableType` in `SegmentGenerator` defaults to `null`.
- [ ] Labels match the specification:
...
- [ ] The variable dropdown is disabled when no type is selected.
- [ ] The variable dropdown shows "Select a variable type first" when disabled.
- [ ] Switched UI order: Type selection comes BEFORE specific variable selection.

...
**Acceptance Criteria**:
- [x] `AdvancedModellingTool.js` handles calculation without clamping results to current values for surplus segments.
- [x] The `finalResult` displayed in the UI is the raw `result` from `calculateModellingDriver`.
- [x] Feasibility signal (Red/Green) still compares the result to the feasible baseline.
...
**Acceptance Criteria**:
- [x] Display informational alert: *"Farmers in this segment already earn more than the income target. In this calculated scenario, incomes would decrease."* when segment income >= target.
- [x] Display warning alert: *"It is not physically possible to reach the income target with the specified model values."* when calculated driver < 0.
- [x] Alerts appear prominently below the calculation result box.
